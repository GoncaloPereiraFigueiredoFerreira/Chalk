...
The storage system is where the actual files are mantained and its funcionalities include storing new files, deleting a file and retrieving files.
- Storing new files: An upload operation is made by sending a single file in the BagIt packaging format.

That file (with a .zip extension) is extracted to a certain folder and the integrity of its data is verified. The files are stored if the verification proves to be successful and are named accordingly to their checksums.

- Deleting a file: A delete operation is made by specifying the location of the file to be erased with a DELETE HTTP method.
